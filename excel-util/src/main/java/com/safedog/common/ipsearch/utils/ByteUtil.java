...
public class ByteUtil {
    ...
    private static final int IP_SECTION = 4;
    ...
    public static String getPath(String fileName) {
        ...
        try {
            filePath = Objects.requireNonNull(ByteUtil.class.getClassLoader().getResource("")).getPath() + fileName;
            File file = new File(filePath);
            if (!file.exists()) {
                // 由于boot项目使用上面方式不能获取到文件方式，因此先进行判断在使用外层目录方式进行获取
